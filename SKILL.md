...
## 字节范（ByteDance 风格）实践规范

> 目标：形成工程化、规范化、语义清晰的代码与协作习惯，减少沟通成本，提升交付质量。

### 工程基线
- Lint 必开：使用 `SwiftLint`（项目根目录 `.swiftlint.yml` 已提供）。CI 或本地 git hook 中执行 `swiftlint`，确保格式统一、无低级风格问题。
- 一处规范，处处遵循：命名、目录结构、接口风格保持一致；相同问题全局同解法。
- 小步快跑，持续重构：每个改动预留 10%–20% 时间做设计与可读性改进；避免战术式“先糊上”。

### 代码与模块
- 深度模块：对外最少接口，对内隐藏细节；以“把复杂度向下拉”为第一目标。
- 数据流单向、可追踪：ViewModel 统一承载 UI 侧状态变更；I/O（剪切板、存储、网络）封装在独立模块。
- 错误定义为不存在：接口在可行处提供“幂等成功”语义（例如清空、删除）。

### 命名与注释
- 命名三要素：精确（Precise）、一致（Consistent）、信息量（Informative）。
- 注释只说代码之外的“意图/约束/边界”；接口注释先行，用来校验设计是否清晰。

### 提交与评审
- 提交信息聚焦“为什么”，而非“做了什么”。
- PR 描述模板：背景/方案/权衡/风险/验证；关联任务与截图（UI/交互变更）。
- 评审关注点：深度模块、信息隐藏、红旗（Shallow Module / Pass-Through / Leakage）。

### 参考 skills（结合 tanweai/pua 仓库经验）
- 抵制战术成功幻觉：别用“先这样吧、之后再改”敷衍；每次改动留下可读、可维护、可回归的痕迹。
- 沟通以事实与数据为准：复现路径、日志、截图、基准；不做主观“感觉上更快”。
- 透明与可溯源：设计、方案、权衡记录在案（代码注释/PR 描述/设计文档片段）。
