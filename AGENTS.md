...
| **本地入口 = 一端口 HTTPS/2**：浏览器只打 `https://127.0.0.1:8080`。Rust `clipvault-http`（hyper + rustls，ALPN `h2` only）是唯一 TCP 监听；Swift origin 在 `$KEEPSAKE_HOME/run/http.sock`。禁止第二浏览器端口、禁止明文 HTTP/1.1、禁止 SwiftNIO/BoringSSL 当边车。`needs_user` 仍 fan-in 进墙 SSE。 | 8080 HTTP/1.1 + 8443 双口；NIOSSL `@rpath/libc++`；手写 HTTP/2 帧 |
