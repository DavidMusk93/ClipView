...
| **View 技术介质**：白纸正文；代码炭黑表面 + JetBrains Mono（自托管 OFL）+ 语言/复制 + View 时重高亮；图/流程图默认浅纸画板 + 题注 + 点击放大。只有整体偏暗的不透明图才用墨井。CSS 在 `archive-view.css`。不改 CAS | Pico 灰底；黑井吞掉深色箭头/标注；正文用等宽体排中文；CDN 拉字体；把高亮写回 archive HTML |
