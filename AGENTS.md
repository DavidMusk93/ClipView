...
| **UI 增量 + lazy**：列表/线程按稳定身份 keyed 调和（笔记预览：lexer 块 hash + React 18；Trae 会话：`event_id` / `layoutKey` patch）。SSE 与保存只补变化。折叠、屏外、未打开的正文 **lazy** 编译或加载。墙卡展开用 overlay / 脱离文档流。会话 bundle 用手风琴**就地**展开，禁止 overlay 盖住后续用户/助手气泡。 | 每次事件/保存整树 `innerHTML`；debounce 冒充增量；关闭 bundle 预渲染全部气泡；会话 item `position:absolute` 盖住对话 |
