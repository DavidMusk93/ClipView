...
| 工具条 | min 36px · 可换行 · 28px 钮 · `#F6F6F8` | 行内标记 **B / I / S** 同一套字母形（`format_bold` / `format_italic` / `format_strikethrough`）。预览也露出工具条；点击先切源码再插入。禁止钮面夹汉字；禁止 `visibility:hidden` 藏条；禁止 `overflow:hidden` 裁标记 |
